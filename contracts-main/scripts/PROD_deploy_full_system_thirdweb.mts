...
const holdingsNov2025CID = "bafkreidppqyr5tmhpjzl2iusz3dddxo5pbiijk2fv3k2cnbzswsy6vligy";
const USDC_ADDRESS = process.env.DEPLOYED_POLYGON_USDC_ADDRESS || "";
...
async function main() {
  console.log(
    `🚀 Deploying to chain ${polygon.name} with account ${account.address}`
  );
  ...
  console.log("Adding USDC as a swappable token in PhiGold...");
  const addUsdcToSwappableTokensTx = prepareContractCall({
    contract: gld,
    method: "addSupportedToken",
    params: [
      USDC_ADDRESS,
      BigInt("100000000"), // 100 USD exchange rate
    ],
  });
  await sendTransaction({ transaction: addUsdcToSwappableTokensTx, account });
  console.log("Done.");
  ...
  const mintTx = prepareContractCall({
    ...
    params: [155000000n, holdingsNov2025CID], // 155 Gld supply
  });
  ...
  fs.writeFileSync(
    "deployment-addresses-polygon.json",
    JSON.stringify(deployments, null, 2)
  );
  console.log(
    "\n🎉 Deployment Complete! Addresses saved to deployment-addresses-polygon.json"
  );
}
